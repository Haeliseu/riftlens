...
const SEVERITY: Record<string, { text: string; msg: TranslationKey }> = {
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      ...
      {isLoading ? (
        ...
        (() => {
          const sev = SEVERITY[data.severity] ?? SEVERITY.warn
          const pct = Math.round(data.ratio * 100)
          return (
            <div className="space-y-2">
              <p className="text-sm">
                {t("coach.obj.stat", {
                  near: data.deathsNearLostObjective,
                  deaths: data.deaths,
                  pct,
                })}
              </p>
              <p className={`text-sm font-medium ${sev?.text ?? ""}`}>
                {t((sev?.msg ?? "coach.obj.warn") as TranslationKey)}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("coach.obj.games", { n: data.games })}
              </p>
            </div>
          )
        })()
